...
const initialMessages: Record<number, { sender: "patient" | "doctor"; text: string; time: string }[]> = {
  1: [
    { sender: "doctor", text: "Hello, how are you feeling today?", time: "09:00" },
    { sender: "patient", text: "I'm feeling better, thank you!", time: "09:01" },
  ],
};

export default function Chat() {
  const [appointmentId, setAppointmentId] = useState(db.appointments[0]?.id || 1);
  ...
  function sendMessage() {
    ...
    setMessages((prev) => ({
      ...
      [appointmentId]: [
        ...(prev[appointmentId] || []),
        { sender: "patient", text: input, time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) },
      ],
    }));
    setInput("");
    ...
    setTimeout(() => {
      setMessages((prev) => ({
        ...
        [appointmentId]: [
          ...(prev[appointmentId] || []),
          { sender: "doctor", text: "Thank you for your message.", time: new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) },
        ],
      }));
    }, 1000);
  }
  ...
  return (
    <div className="max-w-2xl mx-auto py-8 px-4 font-[THICCCBOI]">
      ...
      <div className="mb-4">
        <label className="font-medium text-[#011204] mr-2">Select Appointment:</label>
        <select
      ...
      <div className="bg-white border border-[#E8F6FE] rounded shadow p-4 h-96 overflow-y-auto flex flex-col gap-2 mb-4">
        {(messages[appointmentId] || []).map((msg, idx) => (
          ...
          >
            ...
            >
              <span>{msg.text}</span>
              <span className="text-xs text-right mt-1 opacity-60">{msg.time}</span>
            </div>
          </div>
        ))}
        <div ref={chatEndRef} />
